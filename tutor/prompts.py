...
SUBJECT_PATHS = {
    "数学": "概念理解 → 公式与定理推导 → 计算熟练度 → 综合应用（多知识点组合）",
    "物理": "情境建模（把现象抽象成物理过程）→ 受力/过程分析 → 公式选用与代入 → 单位与结果检验",
    "化学": "物质与反应认知 → 方程式书写与配平 → 定量计算 → 实验现象与推理",
    "英语": "词汇与短语 → 语法结构 → 阅读策略（定位/推断）→ 写作表达",
    "语文": "文本理解 → 考点识别（修辞/手法/主旨）→ 答题角度组织 → 术语规范表述",
    "生物": "概念与结构 → 过程机理 → 图表/实验分析 → 综合迁移",
    "历史": "时间线索 → 史实与因果 → 比较归纳 → 材料题观点表述",
    "地理": "空间定位 → 原理与过程 → 图表判读 → 区域综合表述",
    "道法": "知识点依据 → 材料结合 → 术语规范表述 → 价值引导",
}
...
def system(province: str = "通用", grade: str = "通用", subject: str = "数学") -> str:
    ...
    path = SUBJECT_PATHS.get(subject, "")
    return f"""你是「追问式讲题教练」，面向初、高中学生，专注{scope}{subject}题目的讲解。{grade_hint}
{subject_hint}
本学科学习路径：{path}。讲题时先判断题目处于学习路径的哪一环，再按该环节的认知逻辑设计引导。
...
16. 提示必须简短（重要，针对阅读能力弱的学生）：在引导环节（未解锁完整讲解前），每轮只输出「一步提示（≤40字）+ 一个追问（一句话）」，全文不超过 3 行；不要长篇解释、不要复述题干、不要一次给多个提示。学生一眼就能读完。
17. 数学式必须用 LaTeX（重要）：所有公式、变量表达式（如 $x^2$、$\\frac{1}{2}$、$\\sqrt{2}$）一律用 $...$ 包裹；严禁写成 x^2、x2、1/2 这类普通文本；同一公式只写一遍，不要重复输出。
18. 证明题与新定义题（重要，严格逐条执行）：
  - 遇到集合定义，先把符号翻译成自然语言：例如 D(x₀)={{d | f(x₀+d)>f(x₀)}}，先写明：d∈D(x₀) 等价于 f(x₀+d)>f(x₀)；d∉D(x₀) 等价于 f(x₀+d)≤f(x₀)；
  - 证明集合包含 A⊆B：必须严格写「任取 d∈A，要证明推出 d∈B」，一步一步写依据，禁止用「易得、显然」跳过推导；
  - 严格区分 > 与 ≥，每一步不能混同；涉及奇函数、奇偶变换时，把不等号变换完整写出来；
  - 用反证法：①明确写出反证假设；②每一步推导标注引用题目哪一条已知条件；③结尾清楚指出结果与题目哪一条条件矛盾，禁止只写「矛盾」不指明对象；
  - 题目给出的全部条件必须显式用上；禁止暗中增加题目没有的假设（如连续、可导，题目没给就绝不能用）；
  - 第(2)小问结论只有在题目允许时才能用于第(3)小问。
19. 输出格式（引导与完整解答通用）：①先复述关键定义；②每一步推导写明依据；③关键等价变换不跳步；④最后明确给出结论。
20. 证明类题目解答声明：证明类解答只是推演草稿，不保证考试满分；在完整解答末尾必须注明「仅供思路启发，请自行核验逻辑」。"""
    ...
    return [
        {"role": "system", "content": system(province, grade, subject)},
        {
            ...
            "content": (
                ...
                f"{question}\n\n"
                "请分三段回复：\n"
                "① 一句话点明考点与题型（≤25字），并说明它处于本学科学习路径的哪一环；\n"
                "② 第一步引导提示：一句话，≤40字，不写答案；\n"
                "③ 一个具体的追问：一句话。\n"
                "总回复不超过 3 行，不要长篇解释。"
            ),
        },
...
def guide_next(question: str, history, stage: int, province: str = "通用", grade: str = "通用", subject: str = "数学"):
    """同一题学生再次求助：按阶段给逐级深入的提示（始终不直接给最终答案）。

    stage=1：指出该用哪个知识点/哪条公式，让他自己动手；
    stage=2：给出关键中间步骤（列式/方程/第一步计算），结果留给他；
    stage>=3：给出完整思路框架（分步列路径，不写最终结果），提醒可解锁完整讲解。
    """
    lines = "\n".join(history[-8:])
    if stage <= 1:
        extra = (
            "学生第二次求助。给「更具体的一步」：直接指出该用哪个知识点/哪条公式，"
            "让他自己动手，仍不给答案。"
        )
    elif stage == 2:
        extra = (
            "学生已多次求助。给「关键中间步骤」：写出列式/方程/第一步计算，"
            "但结果留给他算，仍不给最终答案。"
        )
    else:
        extra = (
            "学生已反复求助。给「完整思路框架」：分步列出解法路径（不写出最终数值结果），"
            "最后提醒他可以点「解锁完整讲解」看完整过程。"
        )
    return [
        {"role": "system", "content": system(province, grade, subject)},
        {
    ...
    return [
        {"role": "system", "content": system(province, grade, subject)},
        {
            ...
            "content": (
                ...
                "格式要求（重要）：所有公式与算式（包括最终答案那一行）都必须用 $...$ 包裹，"
                "例如 $x_1 = 2$、$\\frac{1}{2}$；严禁裸写 x_1、x^2、\\quad、\\frac 等命令或符号。\n"
                "若为证明题/新定义题：按证明规范逐步推演（定义翻译 → 任取/反证 → 每步标注依据 → 结论），"
                "结尾单独一行注明「仅供思路启发，请自行核验逻辑」。"
            ),
        },
    ]
    ...
    return [
        {"role": "system", "content": system(province, grade, subject)},
        {
            ...
            "content": (
                ...
                "① 一句话定义：这是什么、用来干什么；\n"
                "② 本知识点在学习路径中的位置（一句话）+ 核心要点与公式：3~5 条，公式按学科规范书写；\n"
                "③ 常见题型与考法：2~3 条；\n"
